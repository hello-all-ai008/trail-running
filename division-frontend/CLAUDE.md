...
- **2026-07-11**: Mobile-first scan stations. Check-in/Check Point/Finish now support camera barcode scanning (`components/scanner/BarcodeCameraScanner.jsx`, lazy-loaded `@zxing/browser`, torch toggle, cooldown, permission/error fallback to manual entry) alongside the existing USB/BT-scanner keyboard-wedge flow — staff pick per device via a toggle in `StationPage.jsx` (default: touch devices → camera, mouse/USB-scanner desks → keyboard, unchanged). Nav (`Sidebar.jsx`) becomes a fixed bottom tab bar below 768px via CSS only (same nav data, no duplication). `.station` grid restructured to `grid-template-areas` so mobile order is scan → LED result → log table (was table-before-LED, wrong priority). Breakpoints consolidated to the project standard 768/1024. `npm run dev` now serves HTTPS (`@vitejs/plugin-basic-ssl`, dev-only) since camera needs a secure context to test over LAN on a real phone. Build/lint/12 unit tests all green; **real-device camera scan + Playwright breakpoint screenshots not yet run** — Chrome extension was disconnected this session, blocked on that or Gong testing on his phone directly.

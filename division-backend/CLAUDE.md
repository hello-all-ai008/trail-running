...
- **2026-07-07 (v1 — live)**: Real backend stood up end-to-end for Steps 4-6. Schema applied (6 tables + `staff_users`, RLS default-deny except `live_progress`), seeded with the real 121-runner database (clean `REGISTERED` state, no scan history — matches `division-frontend`'s dataset). Go service builds clean, `go vet` clean, `internal/scan`'s business rules unit-tested (10 tests, no DB needed — table-driven against a fake `scanHistory`). `division-frontend` wired: `VITE_USE_MOCK_DATA=false` switches `useRaceState.js` from localStorage to this API + Supabase Realtime on `live_progress`; mock path (default) is untouched and still fully localStorage-only.
  **2026-07-07 (project migration):** Moved off the original `trail-running` project (`lhvvfmfjfpfqsaubdiux`, org TIXS, ap-southeast-1 — MCP-created) onto `backend-trail-running` (`emuojzwieluldwvbskxp`, a separate org, **ap-northeast-1**) at Gong's request — he created this project himself in the browser. Same 3 migrations re-applied verbatim, same verification (121/6/2 row counts, advisors all-INFO). Old project paused, not deleted. Note the region change: ap-northeast-1 instead of ap-southeast-1 — slightly more latency from Thailand, not expected to matter at this scale but worth knowing if scan latency ever looks off on race day.
